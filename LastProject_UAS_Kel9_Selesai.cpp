...
vector<bool> kursiTerisi(30, false); // 30 kursi reguler
vector<bool> kursiVIP(10, false);    // 10 kursi VIP
...
Snack* pilihSnack(int& jumlahSnackDipilih) {
    ...
    for (int i = 0; i < jumlahSnackDipilih; ++i) {
        ...
        if (pilihanSnack < 1 || pilihanSnack > jumlahSnack) {
            ...
            --i; // ulang i jika pilihan tidak valid
        } else {
...
void pilihKursi(const string& nama, int umur, const string& judulFilm, int &hargaFilm, const string& lokasi) {
    ...
    if (jenisKursi == 1) {
        ...
        kursiTerisi[nomorKursi - 1] = true; // Tandai kursi sebagai terisi
    ...
    // Memilih kursi VIP
        ...
        kursiVIP[nomorKursi - 1] = true; // Tandai kursi VIP sebagai terisi
        isVIP = true;
        cout << "Kursi VIP nomor " << nomorKursi << " berhasil dipesan." << endl;
        hargaFilm += 20000; // Tambah harga tiket untuk VIP
    } else {
    ...
    delete[] snackPilihan; // Bebaskan memori snack yang sudah dipilih
}
